...
semi_out_int:
	...
	call get_semi_code	
        ...
        CJNE R0, #1h, term;если вывод на вторую лампочку то там ставится точка, если секунд не четное кол-во
        ;она получится по центру
        MOV A, 52h
        MOV B, #2
        DIV AB ;делим что бы получить остаток
        MOV A, B
        ;делим на два и смотрим сколько получилось
        CJNE A, #1, term ;если есть четное то прыгаем дальше
        ;иначе моргаем точкой
        CLR P1.7
...
;шаг таймера
step_time:
        MOV TH0, #0FFh;обновляем настройки таймера
	MOV TL0, #0E0h

	;получаем десятки часов в r1
        MOV A, r6
        MOV B, #0Ah
        DIV AB
        MOV r1,A
	...
	;получаем часов в r2
        ...
        ;получаем десятки минут в r3
        MOV A, r5
        MOV B, #0Ah
        DIV AB
        MOV r3,A
        
        ;получаем минуты в r4
        MOV r4,B

	MOV A, 51h;увеличиваем счетчик милисекунд
        INC A
        MOV 51h, A
        
	CJNE A, #20, OUT ;если не накапала секудна прыгаем дальше
	;иначе обнуляем счетчик милисекунд и увеличиваем секунду
	MOV 51h, #0
	...
	CJNE A, #60, OUT ;если не получили 60 секунд прыгаем дальше
	;иначе обнуляем счетчик и увеличиваем минуты
	MOV 52h, 0
	INC r5
	
        CJNE r5, #60, OUT;Если не получили 60 минут прыгаем дальше
        ;Если получили обновляем счетчик
        MOV r5, #0
	;увеличиваем часы
	INC r6
	
        CJNE r6, #24, OUT;Если не получили 24 часа прыгаем дальше
...
init_timer:
	...
	mov r2, b
	add a, b
	;приводим к максимальному кол-ву часов и сохраняем
	MOV B, #24
	DIV AB
	mov r6, B
	...
	mov r4, b
	add a, b
	;приводим к макс кол-ву минут и сохраняем
	MOV B, #60
	DIV AB
	mov r5, B
